...
ssh-keygen -t ed25519 -N "${client_ca_passphrase}" -f "${client_ca_dir}/${client_ca_filename}" -C "client-ca@${ca_domain}"
echo -e "${BLUE}Done.${NC}\n"

# Sign the client CA public key with the client CA private key
echo -e "${YELLOW}Signing client CA key...${NC}"
ssh-keygen -s "${client_ca_dir}/${client_ca_filename}" -I "client-ca@${ca_domain}" -n "${ca_fqdn}" -V "+${ca_expiration_days}d" -P "${client_ca_passphrase}" "${client_ca_dir}/${client_ca_filename}.pub"
echo -e "${BLUE}Done.${NC}\n"

# Generate a new SSH key pair for the host CA
echo -e "${YELLOW}Generating host CA key...${NC}"
ssh-keygen -t ed25519 -N "${host_ca_passphrase}" -f "${host_ca_dir}/${host_ca_filename}" -C "host-ca@${ca_domain}"
echo -e "${BLUE}Done.${NC}\n"

# Sign the host CA public key with the host CA private key
echo -e "${YELLOW}Signing host CA key...${NC}"
ssh-keygen -s "${host_ca_dir}/${host_ca_filename}" -I "host-ca@${ca_domain}" -h -n "${ca_fqdn}" -V "+${ca_expiration_days}d" -P "${host_ca_passphrase}" "${host_ca_dir}/${host_ca_filename}.pub"
echo -e "${BLUE}Done.${NC}\n"
